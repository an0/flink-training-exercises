...
object Query {
  def apply(longitude: Float, latitude: Float): Query =
    new Query(Random.nextLong, longitude, latitude)
}

object NearestTaxiSolution {

  val queryDescriptor = new MapStateDescriptor[Long, Query](
    "queries",
    createTypeInformation[Long],
    createTypeInformation[Query])
  ...
  class ClosestTaxi
      extends KeyedProcessFunction[Long,
                                   (Long, Long, Float),
                                   (Long, Long, Float)] {
    // store (taxiId, distance), keyed by queryId
    lazy val closetState: ValueState[(Long, Float)] =
      getRuntimeContext.getState(
        new ValueStateDescriptor[(Long, Float)](
          "report",
          createTypeInformation[(Long, Float)]))

    // in and out tuples: (queryId, taxiId, distance)
    override def processElement(
        report: (Long, Long, Float),
        context: KeyedProcessFunction[Long,
                                      (Long, Long, Float),
                                      (Long, Long, Float)]#Context,
  ...
  class QueryFunction
    ...
    override def processElement(ride: TaxiRide,
                                readOnlyContext: KeyedBroadcastProcessFunction[
                                  ...
                                  TaxiRide,
                                  Query,
                                  (Long, Long, Float)]#ReadOnlyContext,
                                out: Collector[(Long, Long, Float)]): Unit =
      if (!ride.isStart)
        for (entry: Entry[Long, Query] <- readOnlyContext
               .getBroadcastState(queryDescriptor)
               .immutableEntries()) {
    ...
    override def processBroadcastElement(
        query: Query,
        context: KeyedBroadcastProcessFunction[Long,
                                               TaxiRide,
                                               Query,
                                               (Long, Long, Float)]#Context,
        out: Collector[(Long, Long, Float)]): Unit = {
